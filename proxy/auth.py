...
class UserManager:
    ...
    def _load(self):
        ...
        if existing and exist_key != expected:
            existing.api_key = expected
            del self._users[exist_key]
            self._users[expected] = existing
            self._save()
            print(f"[Migrate] Admin key -> {expected}")
        ...
        if not existing:
            ...
            self._save()
            print(f"[Init] Created admin user, API Key: {expected}")
